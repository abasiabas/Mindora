...
export interface SafetyCheckResult {
  ...
  userMessage?: string; // shown instead of / alongside the AI response
}

// ── Pattern sets (Persian + English). Deliberately kept at a general,
// non-exhaustive level — this is a routing signal, not a script. ──────────
...
/**
 * Runs the full safety scan on a single user message. This MUST be called
 * server-side, before any AI response is generated (bind 4 "چند لایه
 * enforce شود": Input Safety here; Response/Output Safety happen again on
 * the generated reply before it's shown to the user — see validateResponse).
 */
export function scanUserMessage(text: string): SafetyCheckResult {
  const events: SafetyCheckResult["events"] = [];

  if (matchesAny(text, SUICIDE_RISK_PATTERNS)) {
    events.push({ type: "suicide_risk", severity: "critical" });
  }
  if (matchesAny(text, SELF_HARM_PATTERNS)) {
    events.push({ type: "self_harm", severity: "high" });
  }
  if (matchesAny(text, ABUSE_PATTERNS)) {
    events.push({ type: "abuse", severity: "high" });
  }
  if (matchesAny(text, MEDICATION_PATTERNS)) {
    events.push({ type: "medication_request", severity: "medium" });
  }
  if (matchesAny(text, DIAGNOSIS_REQUEST_PATTERNS)) {
    events.push({ type: "diagnosis_request", severity: "low" });
  }
  if (matchesAny(text, OUT_OF_SCOPE_HINTS)) {
    events.push({ type: "out_of_scope", severity: "low" });
  }
...
/**
 * Persists safety events to the audit trail (bind 4 layer 6: Database /
 * Audit Logging). Uses the admin client because safety_events has no client
 * insert policy — only the server-side pipeline may write here, so a
 * compromised or manipulated client request can never fabricate/erase a
 * safety event.
 */
...
/**
 * Output-side check (bind 4 layer 5: Response Validation). Even if the
 * user's message passed input safety, the generated AI response itself
 * must never contain medication names/dosage or fabricated citations.
 * This is a second, independent gate — never trust a single layer.
 */
